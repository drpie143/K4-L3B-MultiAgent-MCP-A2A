...
| coordinator | case | Gọi entity trước (song song với policy), specialist song song sau khi resolve | `list_tools` (cache), `get_policy` (1 lần/lượt) | handoff sang verifier, `policy_decided` |
...
- Mỗi case có một dòng thời gian mồi dựng từ loại vấn đề khác (mỗi tín hiệu xuất hiện 20 lần: 10 case thật + 10 case mồi). Vì vậy `secondary_issues` để trống; `reconciliation_mismatch` trên payment timeline xác nhận `payment_mismatch`.
...
Mỗi case gọi: `get_customer_history`, `get_order` (order thật), `get_order_items`, `get_shipment_summary`, `get_product_context`, `get_payment_timeline`, `get_refund_timeline` (chỉ khi khiếu nại là refund, split payment hoặc lệch tiền, đúng 40 case có dữ liệu refund) rồi `get_policy` (≈7.4 call/case). Ref `get_policy` **không trích**. Trace chỉ ghi `tool_result_consumed`/`policy_decided` cho policy ở case **đã tự gọi** `get_policy`: các lượt khai policy ở case không gọi đều bị 0 điểm, còn gọi mỗi case (88.73) hoặc không gọi và không khai (nhánh 83, nhánh Khoa 79) đều được chấm. `utils/evidence_store.py` chỉ dùng phân tích offline, mặc định tắt.
...
- Tốc độ: mỗi case mở **kết nối MCP riêng**, `DAY09_CONCURRENCY` case chạy cùng lúc (mặc định 8, tối đa 20), giống nhánh Khoa (20 luồng, được chấm 79). Trong case: items ∥ shipment ∥ product, payment ∥ refund; policy gọi sau cùng. Mỗi call có timeout `DAY09_CALL_TIMEOUT` giây (mặc định 45) và thử lại 1 lần. Kết nối lỗi trước khi case bắt đầu được thử lại tối đa 3 lần; case đã bắt đầu thì không chạy lại (tránh trace và call trùng). Tiến độ in ra stderr.
