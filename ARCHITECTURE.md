...
| coordinator | case | Gọi entity trước, specialist song song sau khi resolve | `list_tools` một lần để giới hạn entity | handoff sang verifier |
| verifier | ba result | Chọn primary issue, ghi conflict, dựng output | không gọi MCP | dict đúng schema |
...
`customer_unique_id_hint` trên case không được dùng làm điều kiện loại order. Định danh khách lấy từ evidence `get_order`, rồi mới gọi history.
