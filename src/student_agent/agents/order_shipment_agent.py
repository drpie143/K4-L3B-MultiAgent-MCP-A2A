...
async def investigate_order_shipment(
    ...
    for order_id in entity.resolved_order_ids:
        ...
        if isinstance(shipment_payload, dict):
            ...
            if ref:
                refs.append(ref)
        shipment_data = shipment_payload.get("data") if isinstance(shipment_payload, dict) else None
        ...
        if isinstance(item_payload, dict):
            ...
            if ref:
                refs.append(ref)
        item_data = item_payload.get("data") if isinstance(item_payload, dict) else None
        views.append(_analyze(order_data, item_data, shipment_data, opened_at=opened_at))
